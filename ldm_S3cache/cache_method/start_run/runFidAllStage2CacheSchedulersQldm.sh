...
# Q-LDM + S3-Cache: run cache FID for all three Stage2 refined schedulers.
# Mirrors runFidAllStage2CacheSchedulersLdm.sh with --cali_ckpt and *_qldm outputs.
...
if [[ ! -f "${CALI_CKPT}" ]]; then
  echo "[ERROR] missing Q-LDM cali_ckpt: ${CALI_CKPT}" >&2
  exit 1
fi
...
mkdir -p "${LOG_ROOT}"
BATCH_LOG="${LOG_ROOT}/qldm_stage2_cache_all_$(date +%Y%m%d_%H%M%S).log"
...
run_cache() {
  ...
  echo ""
  echo "============================================================"
  echo "Q-LDM cache run: ${scheduler_name}"
...
{
  echo "============================================================"
  echo "Q-LDM + S3-Cache: all Stage2 cache schedulers"
  ...
  echo ""
  echo "[DONE] all three Q-LDM cache runs complete"
